...
public partial class OLApply_Apply4 : System.Web.UI.Page
{
    ...
    protected void Load_Data()
    {
        ...
        ppUsePoint.Text = Convert.ToString(Session["ppUsePoint"]);
        ppMustPoint.Text = Convert.ToString(Session["ppMustPoint"]);
        ppRemainPoint.Text = Convert.ToString(Session["ppRemainPoint"]);
        ...
        if (!string.IsNullOrEmpty(Price_Show_1.Text)) value1 += int.Parse(Price_Show_1.Text, NumberStyles.AllowThousands);
        if (!string.IsNullOrEmpty(Price_Show_2.Text)) value1 += int.Parse(Price_Show_2.Text, NumberStyles.AllowThousands);
        if (!string.IsNullOrEmpty(Price_Show_3.Text)) value1 += int.Parse(Price_Show_3.Text, NumberStyles.AllowThousands);
    ...
    {
        string strSql = "";
        strSql += " SELECT name ,pager ,compno ";
        strSql += " FROM IND_FG ";
        ...
        //strSql += " SELECT Sales ,pager, compno  ";
        //strSql += " FROM IND_FG ";
        //strSql += " left join AGENT_M on IND_FG.IND_FG_CompNO = AGENT_M.COMP_NO ";
        //strSql += " left join Person on AGENT_M.Sales = Person.name ";
        //strSql += " where IND_FG_ID = @IND_FG_ID ";
        string strConnString = System.Web.Configuration.WebConfigurationManager.ConnectionStrings["B2BConnectionString"].ToString();
        SqlConnection connect = new SqlConnection(strConnString);
